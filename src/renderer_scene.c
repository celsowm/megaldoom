...
// Mixed-tilemap scratch buffer (only exists at flag 1; file scope so both the
// call site and the relocated definitions can see it).
...
// === relocated to file scope: was nested in #if DEBUG_PERF (broken plain flag-1 builds) ===
#if RENDERER_SPARSE_FB
