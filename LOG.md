...
## BSP early rejection: small cast win, split attribution fixed (2026-09-20)

The first executable step from the performance plan moved the horizontal
viewport/sample-coverage rejection in `bsp_render_columns.c` ahead of the
per-segment reciprocal, texture-U and interpolation setup. `BSP_EARLY_REJECT`
is a compile-time A/B switch and defaults on. The old order remains available
with `-DBSP_EARLY_REJECT=0` so both builds can be made from the same tree.

Pose-locked E1M6 horde, `(x,y)=(-1984,-1392)`, angle 9, level 5, 3200 frames:

| build | frame vb | cast | pack | projection | billboard |
|---|---:|---:|---:|---:|---:|
| early rejection off | 24.38 | 6528 | 3530 | 4556 | 7836 |
| early rejection on | **24.22** | **6329** | 3527 | 4549 | 7843 |

The cast change is -3.0% (199 subticks), or 0.16 vblank in this view. The
world-state counters were the same (73 segments tested, 17 boxes, 12 nodes),
and the billboard difference is enemy timing noise, so this is a real but small
win. It is not a claim of a whole-FPS improvement.

The same-session E1M7 full-PVS pose `(96,1232)`, angle 137, level 6, repeated
the direction of the change: 8549 -> **8068** cast subticks (-5.6%), with
5680 pack, 97 tested segments and 26 boxes identical. The frame moved
24.38 -> **24.02** vblanks. This remains a sub-vblank optimization, but it is
repeatable in two materially different heavy views.

The `CADENCE_DRAWSEG_SPLIT` timer was also moved into the public segment draw
wrappers. Previously it only surrounded `bsp_visit_leaf`; the baked program
calls those wrappers directly, so its report showed 97 segments tested but
`drawseg total = 0`. On E1M7 `(96,1232)`, angle 137, the repaired report reads
97 segments tested, 13 drawn, `drawseg total = 7154` and `sample loop = 3674`
subticks per rebuild. GROUP box calls are still part of the traversal total and
need a separate bracket if they are to be attributed independently.

**Correctness.** The E1M6 visibility oracle with early rejection on compared
1377 program frames against traversal with zero mismatches; the route completed
(1129/1129). The negative bake control (`--negative-control-drop-every 7`)
reported 724 mismatches in 1364 frames and the route completed. The normal bake
was regenerated byte-identically afterward. `test-bsp-render-math.py` and
`test-billboard-projection.py` pass. Release build guardrails pass with 21,566
bytes of work RAM free and resident end `0x1EF388`.

This is worth keeping as a low-risk micro-optimization, but it is not the
structural BSP improvement sought by the plan. The next BSP work should use the
repaired attribution before attempting directional PVS variants.

### Overlay depth rejection: no measurable win at the window pose

As a follow-up, the same cast loop was temporarily changed to test a nearer
door/window overlay immediately after `depth_col`, before calculating U and
vertical height for a farther overlay sample. The A/B was pose-locked at the
E1M1 window pose `(1300,3300)`, angle 0, level 0, 3200 frames:

| build | frame vb | cast | pack | projection | billboard |
|---|---:|---:|---:|---:|---:|
| early overlay reject off | 14.34 | 8009 | 7480 | 1075 | 370 |
| early overlay reject on | 14.34 | 8017 | 7477 | 1077 | 369 |

The scene has no useful population of farther overlay samples behind a nearer
one, so the potential work was not present. The temporary change was reverted;
the default early segment rejection remains.

## Directional PVS prototype: four bins pass E1M6 oracle (2026-09-20)

`tools/analyze-bsp-directional.py` now reuses the production `leaf_program`
builder and has a conservative interval mode. It expands each PVS leaf set to
its actual emitted SEG indices, then takes the angular arc of the vector AABB
formed by the entire player-cell AABB and each segment's endpoint AABB. A SEG is
kept whenever that over-approximation intersects the bin's 90-degree frustum;
facing, near clipping and exact interpolation are ignored, so uncertainty keeps
extra work rather than removing it.

The corrected interval study gives E1M6 53,656–57,545 unique words per bin for
four 64-angle bins (about 52,570–58,476 words before deduplication), versus
84,684 words for the all-heading program. E1M7 gives 39,600–43,285 words per
bin versus 78,526. Four bins are the practical first prototype: eight bins
would overflow E1M6's 1.5 MB bank once the existing wall pack and baseline
program are included.

The generated E1M6 prototype was connected behind `BSP_VIS_DIRECTIONAL=1` and
`BSP_VIS_DIRECTIONAL_BIN_SHIFT=6`, with the ordinary program as fallback for
maps without directional data. The full E1M6 route completed (1129/1129) with
749,995 frames and **zero oracle mismatches**. A deliberate corruption of every
directional SEG word in all four bins reported 64 mismatches in 583 checked
frames, proving the oracle path was actually exercising the prototype. The
generated prototype files were removed afterward; the selector and generator
remain guarded/off by default.

This is a real candidate for the next performance measurement, but it is not
yet enabled in the shipped bake. The route proof covers E1M6 only; all campaign
maps need four-bin data, per-pack size checks and their own complete oracle
