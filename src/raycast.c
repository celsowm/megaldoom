...
static u16 cast_single_ray(const PlayerState *player, u16 angle, u8 *texture_id, u8 *shade, u8 *tex_x) {
    ...
    for (u16 step = 0; step < DDA_MAX_STEPS; step++) {
        ...
        if (tile != 0) {
            ...
            *tex_x = (u8)(wall_coord >> 4);
            *shade = side;
            if (tile == WORLD_TILE_DOOR) {
                *texture_id = RAY_TEXTURE_DOOR;
            } else if (tile == WORLD_TILE_LOCKED_DOOR) {
                *texture_id = RAY_TEXTURE_LOCKED_DOOR;
            } else if (tile == WORLD_TILE_EXIT_SWITCH) {
                *texture_id = RAY_TEXTURE_EXIT;
            } else if (tile == WORLD_TILE_WALL_BROWN) {
                *texture_id = RAY_TEXTURE_WALL_BROWN;
            } else if (tile == WORLD_TILE_WALL_GRAY) {
                *texture_id = RAY_TEXTURE_WALL_GRAY;
            } else if (tile == WORLD_TILE_WALL_METAL) {
                *texture_id = RAY_TEXTURE_WALL_METAL;
            } else if (tile == WORLD_TILE_WALL_BRICK) {
                *texture_id = RAY_TEXTURE_WALL_BRICK;
            } else if (tile == WORLD_TILE_WALL_TECH) {
                *texture_id = RAY_TEXTURE_WALL_TECH;
            } else {
                *texture_id = RAY_TEXTURE_WALL;
            }
...
void raycast_cast_frame(const PlayerState *player, RayColumn *columns) {
    ...
    for (u16 col = 0; col < RAY_VIEW_COLS; col += RAY_COL_STRIDE) {
        ...
        u8 shade;
        u8 tex_x;
        const u16 dist = cast_single_ray(player, ray_angle, &texture_id, &shade, &tex_x);
        ...
        height = (u16)((RAY_VIEW_PIXEL_H * FX_ONE) / corrected_dist);

        if (height < 1) {
            height = 1;
        } else if (height > RAY_VIEW_PIXEL_H) {
            height = RAY_VIEW_PIXEL_H;
        }
