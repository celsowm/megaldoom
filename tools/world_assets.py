...
# Distance-fog steps baked into every wall pack block. Emitted as
...
# Shade-ready pair planes live in banked ROM, one pack per level (see
# tools/md_banked.ld and src/bsp/level_bank.c). A pack is two runs of 32 KB
# texture blocks -- the level's wall textures, then its door-framed textures --
# each laid out [shade][tex_x][tex_y], the byte order the old monolithic
# FREEDOOM_WALL_PACKED_PAIRS[shade][texture][tex_x][tex_y] had per texture.
# The runtime reaches a block through a resident per-level table of block
# addresses indexed by the global texture id, so seg data and every
...
def wall_pair_block(rows_by_shade, meta):
    """One texture's plain wall pairs for every shade level."""
    out = bytearray()
    v_scale_q12 = meta["v_scale_q12"]
    level = WALL_IDENTITY_MAP
    for shade in range(WORLD_SHADE_LEVELS):
        rows = rows_by_shade[shade]
        for tex_x in range(WALL_TEX_WIDTH):
            for tex_y in range(WALL_TEX_HEIGHT):
                row = rows[(tex_y * v_scale_q12) >> 12]
                out.append(packed_pair_byte(level, row[2 * tex_x],
                                            row[2 * tex_x + 1]))
    return bytes(out)


def door_pair_block(rows_by_shade, meta):
    """One texture's door-framed pairs for every shade level."""
    out = bytearray()
    level = WALL_IDENTITY_MAP
    source_height = meta["height"]
    v_scale_q12 = meta["v_scale_q12"]
    border = WALL_TEX_WIDTH // 16
    safety = source_height // 8
    frame_height = source_height // 16
    for shade in range(WORLD_SHADE_LEVELS):
        rows = rows_by_shade[shade]
        for tex_x in range(WALL_TEX_WIDTH):
            for tex_y in range(WALL_TEX_HEIGHT):
                ...
                out.append(packed_pair_byte(level, left, right))
    return bytes(out)
...
def emit_level_wall_packs(level_packs, asm_path, texture_names, texture_ids,
    ...
    for index, (map_name, wall_names, door_names, pack_path, incbin_path) in \
        ...
        if unknown:
            raise ValueError("%s uses textures absent from the catalog: %s" %
                             (map_name, ", ".join(sorted(unknown))))
        walls = [name for name in texture_names
                 if name == FALLBACK_TEXTURE or name in wall_names]
        doors = [name for name in door_texture_names if name in door_names]
        missing_doors = set(door_names).difference(doors)
        if missing_doors:
    ...
    asm.extend(base_table("megaldoom_level_wall_bases", wall_rows))
    asm.extend(base_table("megaldoom_level_door_bases", door_rows))
    asm.append("")
    with open(asm_path, "w", newline="\n") as fh:
        fh.write("\n".join(asm))

    lines = [
        "// Banked wall packs (tools/world_assets.emit_level_wall_packs). Level",
        "// L's pack is mapped at the 0x280000 level window while L is loaded",
        "// (src/bsp/level_bank.c): its wall blocks, then its door-framed blocks,",
        "// each MEGALDOOM_LEVEL_PACK_BLOCK_BYTES laid out [shade][tex_x][tex_y].",
        "// Block addresses per level and texture id are in",
...
def emit_world_assets(path, texture_usage, sectors, provenance=None,
    ...
    lines.extend([
        ...
        "// remap any more: each shade plane of a wall pack block is",
