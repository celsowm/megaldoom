...
n = min(S, ROWS). ROWS is the table's row width (120): the 128-row viewport
preset indexes past it for S > 120, and those columns keep the generic post.
...
def main():
    ...
    heights, rows, table = load_table(ASSETS.read_text())
    asm = [
    ...
    for s in range(1, heights):
        n = min(s, rows)
        samples = [table[s][i] & TEX_MASK for i in range(n)]
    ...
    header = [
        ...
        "#define MEGALDOOM_WALL_SCALER_ROWS %d" % rows,
