...
with tempfile.TemporaryDirectory(prefix="megaldoom-build-test-") as folder:
    ...
    source.mkdir()
    for index, name in enumerate((*frontend.PATCHES, *frontend.GLYPHS,
                                  *frontend.INTERMISSION_PATCHES), start=1):
    ...
    # three-map campaign, 46 with E1M4, 48 with E1M5; Sonic's two boot sprite
    # sheets make 50.
    # The CONTROLS submenu adds one label panel per cursor row plus the
    # button-letter sheet.
    assert len(frontend.EXPECTED_OUTPUTS) == 50 + (frontend.CONTROLS_ROWS + 1) + (
