...
static void remove_index_stable(u16 *indices, u16 *count, u16 index) {
...
void billboard_registry_add(u16 index) {
    ...
    s_active_indices[s_active_count++] = index;
    if (g_billboards[index].type_id == BILLBOARD_TYPE_DUMMY) {
        s_enemy_indices[s_enemy_count++] = index;
    ...
    if (type->targetable) {
        s_target_indices[s_target_count++] = index;
    }
    if (type->blocking) {
        s_blocking_indices[s_blocking_count++] = index;
    }
}
...
const u16 *billboard_registry_active_indices(void) { return s_active_indices; }
const u16 *billboard_registry_enemy_indices(void) { return s_enemy_indices; }
const u16 *billboard_registry_target_indices(void) { return s_target_indices; }
const u16 *billboard_registry_blocking_indices(void) { return s_blocking_indices; }
